...
- GTO 比較需核實完整来源及牌譜模型。當前 adapter 無法核對相同 rake，歷史比較一律只供參考，不納入偏差或正誤評核；位置、路徑、籌碼等差異逐項顯示。內部一致的理論頻率可作練習；祖先疑義會傳遞到下游牌組，舊包與舊題卡讀取時亦重新檢查。
...
Schema 3 增加 `study_decisions`、`study_indexed`、`study_items`、`study_packs`、`study_cards`、`study_sessions`、`study_attempts`。穩定參照保留 `hero-decisions/2`；衍生索引升為 `hero-index/3`，先備份再分批重建。既有帳本、原文、筆記及統計不被衍生索引覆蓋。
